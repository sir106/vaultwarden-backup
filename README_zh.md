...
遵循最佳实践的 GFS（祖父-父-子）备份保留策略，支持按天、周、月、年和最近备份数量进行灵活保留。将所有保留变量均设置为 `0` 则保留所有备份文件（禁用清理）。
 
- **`BACKUP_KEEP_DAYS`**：保留最近 X 天的每日备份（每天保留最新的一个备份）。
  - 默认值：`7`
- **`BACKUP_KEEP_WEEKS`**：保留最近 X 周的每周备份（ISO 周，每周保留最新的一个备份）。
  - 默认值：`4`
- **`BACKUP_KEEP_MONTHS`**：保留最近 X 个月的每月备份（每月保留最新的一个备份）。
  - 默认值：`12`
- **`BACKUP_KEEP_YEARS`**：保留最近 X 年的每年备份（每年保留最新的一个备份）。
  - 默认值：`3`
- **`BACKUP_KEEP_LAST`**：保留最近 X 次备份（无论日期）。
  - 默认值：`0`
