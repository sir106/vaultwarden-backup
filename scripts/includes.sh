...
function init_env() {
    ...
    if [[ "${INIT_ENV_LOG}" != "FALSE" ]]; then
        ...
        if [[ "${BACKUP_DASHBOARD_ENABLE}" == "TRUE" ]]; then
            ...
            elif [[ "${BACKUP_DASHBOARD_ADMIN_TOKEN}" =~ ^\$argon2 || "${BACKUP_DASHBOARD_ADMIN_TOKEN}" =~ ^\$\$argon2 || "${BACKUP_DASHBOARD_ADMIN_TOKEN}" =~ ^[\'\"]\$argon2 ]]; then
                color yellow "BACKUP_DASHBOARD_ADMIN_TOKEN: ${#BACKUP_DASHBOARD_ADMIN_TOKEN} Chars (Argon2 hash detected - enter your plain password in the frontend UI)"
            else
                color yellow "BACKUP_DASHBOARD_ADMIN_TOKEN: ${#BACKUP_DASHBOARD_ADMIN_TOKEN} Chars (Plain password detected)"
            fi
        fi
...
function init_env_dashboard() {
    ...
    if [[ "${BACKUP_DASHBOARD_ENABLE^^}" == "TRUE" ]]; then
        export BACKUP_DASHBOARD_ENABLE="TRUE"
    else
        export BACKUP_DASHBOARD_ENABLE="FALSE"
    fi
    export DASHBOARD_ENABLE="${BACKUP_DASHBOARD_ENABLE}"

    # BACKUP_DASHBOARD_BIND_ADDR
    get_env BACKUP_DASHBOARD_BIND_ADDR
    export BACKUP_DASHBOARD_BIND_ADDR="${BACKUP_DASHBOARD_BIND_ADDR:-"0.0.0.0"}"
    export DASHBOARD_BIND_ADDR="${BACKUP_DASHBOARD_BIND_ADDR}"

    # BACKUP_DASHBOARD_PORT
    get_env BACKUP_DASHBOARD_PORT
    export BACKUP_DASHBOARD_PORT="${BACKUP_DASHBOARD_PORT:-"8080"}"
    export DASHBOARD_PORT="${BACKUP_DASHBOARD_PORT}"

    # BACKUP_DASHBOARD_ADMIN_TOKEN
    get_env BACKUP_DASHBOARD_ADMIN_TOKEN
    export BACKUP_DASHBOARD_ADMIN_TOKEN="${BACKUP_DASHBOARD_ADMIN_TOKEN}"
    export DASHBOARD_ADMIN_TOKEN="${BACKUP_DASHBOARD_ADMIN_TOKEN}"
}
