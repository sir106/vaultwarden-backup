...
# Test dashboard functionality with proper rclone configuration
cd "$(dirname "$0")" || exit 1
...
RCLONE_CONFIG_DIR="$(pwd)/config"
RCLONE_CONFIG_FILE="${RCLONE_CONFIG_DIR}/rclone.conf"

mkdir -p "${RCLONE_CONFIG_DIR}"

# Create a minimal rclone config with local remote
...
docker run --rm \
    ...
    --mount "type=bind,source=${RCLONE_CONFIG_DIR},target=/root/.config/rclone/" \
